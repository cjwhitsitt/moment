...
- Q: Operator View Frame Timing & Stitch Length Configuration Calculation → A: The operator view modal configuration screen allows adjusting the frame duration (in 50ms increments) and overall stitch length (in steps of 50ms * total frames), where modifying one dynamically recalculates the other in milliseconds. The total frame count for the calculation is based on the full ping-pong loop sequence (2N - 2 frames for N active cameras, e.g. 8 frames for 5 cameras).
- Q: Frame Duration Range Limits & Default → A: The frame duration defaults to 100ms, with an allowed minimum bound of 50ms (20 fps) and a maximum bound of 500ms (2 fps). Any manual text entry outside this range is clamped or validated to stay within the [50ms, 500ms] boundary (and stitch length proportionally bounded).
- Q: Operator View Timing Modal Access & Persistence → A: The timing configuration modal is accessible via a settings/gear icon in the Operator Dashboard header. The configured frame duration is persisted locally across app launches using SharedPreferences, defaulting to 100ms if no prior setting is saved.
- Q: Baseline Camera Count for Stitch Length & Anchor Persistence → A: When fewer than 3 cameras are connected, the total stitch length calculation defaults to using the minimum baseline of 3 cameras. The individual frame duration is the sole anchor value persisted in local storage; total stitch length is always derived from the persisted frame duration and active camera count.
- Q: Frame Timing Transmission to Stitching Pipeline → A: The configured frame_duration_ms is sent as part of the trigger capture WebSocket message payload from the Operator App to the Go Coordinator. The Coordinator persists frame_duration_ms into the Firestore session document, which the Cloud Functions FFmpeg stitching worker reads to set the exact frame delay/framerate for the generated ping-pong GIF.

### Session 2026-09-09

- Q: Force Cloud Resources Configuration → A: The Flutter client application supports forcing live Firebase cloud resources (Firestore, Storage, Functions) instead of local emulators in debug builds via the build-time compilation flag `--dart-define=USE_CLOUD_RESOURCES=true`. When this flag is enabled, emulator redirection is bypassed and the application communicates directly with production Firebase cloud infrastructure.
- Q: Guest Share URL Resolution under Forced Cloud Resources → A: When the application is compiled with `--dart-define=USE_CLOUD_RESOURCES=true`, the guest sharing QR code and landing page URL resolution on the Operator App automatically targets the production Firebase Hosting domain (`https://moment-aad8b.web.app/?gif=...`) instead of the local coordinator emulator address on port 5000.
...
- **FR-034**: The Operator App MUST provide a modal configuration screen accessible via a settings/gear icon in the Operator Dashboard header, allowing the operator to adjust the frame time length (duration per frame) and total stitch length (overall animation duration) in milliseconds using editable text fields paired with decrease and increase buttons. The frame time length MUST default to 100ms with a minimum of 50ms and a maximum of 500ms, and MUST serve as the sole anchor value persisted locally across app launches using SharedPreferences. Adjusting the frame time length MUST step in 50ms intervals and dynamically recalculate the total stitch length, while adjusting the total stitch length MUST step in increments of $50\text{ms} \times (2N - 2)$ (where $N = \max(3, \text{active camera count})$, representing the full ping-pong loop count defaulting to a minimum of 3 cameras when $<3$ cameras are connected) and dynamically recalculate the frame time length. Modifying either field manually via text entry MUST validate/clamp to the allowable range and immediately recalculate the paired field. Total stitch length MUST dynamically re-derive whenever camera nodes connect or disconnect. The Operator App MUST include `frame_duration_ms` in the capture trigger WebSocket message payload sent to the Coordinator; the Coordinator MUST persist this value in the Firestore session document; and the Cloud Functions stitching pipeline MUST read `frame_duration_ms` to configure the FFmpeg frame delay/framerate when assembling the final GIF.
...
- **Capture Session**: Represents a single synchronized trigger event, containing the captured frames from all active nodes (3 to 10), a configured `frame_duration_ms` playback rate, a status, and a reference to the final stitched animation.
