...
│   └── send-gif-email.md # Callable Cloud Function & secrets contract
...
20. Start the client in Camera Node mode, rotate the device between portrait and landscape, and verify the "Moment Camera Node" AppBar header dynamically hides in landscape and shows in portrait.
21. Launch the Firebase Hosting emulator, trigger a capture session, scan the generated guest QR code, and verify that it opens the local emulator hosting page on port 5000 showing the captured GIF and initiating a file download on click.
22. Complete a capture session, verify that the Camera Node screen displays the full sensor viewport with a translucent shade guiding the active 16:9/9:16 zone, download the raw JPEG from Storage and verify it is uncropped, and confirm the final stitched GIF in the Operator dashboard renders at 16:9 (800x450) without black bars.
23. Complete a capture session, verify that the operator sharing view fits entirely on one screen without scrolling, and confirm the preview GIF centers and scales correctly in both portrait and landscape configurations.
24. Complete a capture session, tap the stitched GIF, confirm it zooms in a dismissible modal overlay, and verify tapping the guest sharing QR code displays a large zoomed version that closes on tap.
25. Connect to a fresh session, verify the pairing QR code auto-opens, dismiss it, tap the Add Node button to manual-open it, pair 4 nodes, and confirm the grid column count scales responsively when changing screen width.
26. Complete a capture session, confirm the Guest QR code and "Email" share button are side-by-side, tap the "Email" button, enter a recipient, send the email, and verify status updates are displayed inside the overlay.
27. Tap the settings gear icon in the Operator Dashboard header, verify the Animation Timing modal opens with linked Frame Duration and Total Stitch Length fields, test decrease/increase buttons in 50ms (and $50\text{ms} \times (2N - 2)$) increments, verify manual text entry recalculation and boundary clamping ([50ms, 500ms]), verify local persistence in SharedPreferences across app restarts, and confirm triggered captures stitch at the configured duration.
28. Build and launch the mobile app with `fvm flutter run --dart-define=USE_CLOUD_RESOURCES=true`. Connect to the local Go coordinator, enter Operator Mode, and verify that no "EMULATOR" badge is displayed on the dashboard, that guest sharing QR codes point to `https://moment-aad8b.web.app/?gif=...`, and that Firestore and Storage operations communicate directly with production Firebase cloud resources. Relaunch without `--dart-define=USE_CLOUD_RESOURCES=true` in debug mode, and verify that the "EMULATOR" badge is visible on the Operator Dashboard and local emulator endpoints are used.
29. Configure `RESEND_KEY` in `functions/.secret.local` for local emulator testing and verify that `sendGifEmail` successfully reads the secret and dispatches emails. Verify that if `RESEND_KEY` is missing or unset, `sendGifEmail` rejects the call with a `failed-precondition` error.
